...
final class BufferedStream : StreamBase
{
                                ...
                                ref ubyte[] arr)
    {
        ...
        while(status == size_t.max)
        {
            ...
            auto nread = _input.read(arr[avalid..$]);
            if(!nread)
            {
                // no more data available from the stream, process the EOF
     ...
     */
    size_t delegate(ubyte[] data) decoder() @property
    {
        return _decode;
    }
    ...
    void put(const(ubyte)[] data)
    {
        ...
        auto fcheck = _flushCheck ? _flushCheck(data) : -1;
        assert(fcheck <= cast(ptrdiff_t)data.length);
        // minwrite is the number of bytes that must be written to the
        // stream.
        ptrdiff_t minwrite = (fcheck < 0) ? 0 : fcheck + pos;
        if(minwrite > 0 || pos + data.length >= buffer.length)
        {
            if(pos + data.length - minwrite >= buffer.length)
            {
            ...
            if(_encode)
            {
                ...
                // buffer already.
                while(data.length + pos >= buffer.length)
                {
                    // already data in the buffer, this is a special case, we
                    // can optimize the rest in the loop.
                    auto ntoCopy = buffer.length - pos;
                    auto totalbytes = pos + ntoCopy;
                    buffer[pos..totalbytes] = data[0..ntoCopy];
                    data = data[ntoCopy..$];
                    ...
                    if(totalbytes != nencoded)
                    {
                        ...
                        // the buffer
                        pos = totalbytes - nencoded;
                        _startofwrite = nencoded > _startofwrite ? 0 :
                            _startofwrite - nencoded;
                        memmove(buffer.ptr, buffer.ptr + nencoded, pos);
                    }
                    else
                    {
                        pos = _startofwrite = 0;
                ...
                if(minwrite > 0)
                {
                    ...
                    auto ntoCopy = minwrite - pos;
                    buffer[pos..minwrite] = data[0..ntoCopy];
            ...
            else
            {
                ...
                while(data.length > 0)
                {
                    if(pos) // still data in the buffer
                    {
                        ...
                        else
                        {
                            ...
                            if(minwrite > 0 && minwrite <= buffer.length)
                            {
                                ...
                                buffer[pos..minwrite] = data[0..minwrite - pos];
                                ensureWrite(buffer[0..minwrite]);
                                // update state
                                data = data[minwrite - pos..$];
                                minwrite = 0;
                                pos = 0;
                            }
                            else
    ...
    // private version, avoids duplicate mode checks
    private void _flush_noModeCheck()
    {
        if(pos)
        {
            if(_encode)
            {
                auto nencoded = _encode(buffer[0..pos]);
                ensureWrite(buffer[0..nencoded]);
                if(pos != nencoded)
                {
                    // need to move the non-encoded bytes to the front of
     ...
     * additional feature that if you seek within the buffer, while in read
     * mode, the function succeeds even if the underlying device is not
     * seekable.
     *
     * In write mode, the buffer is always flushed before seeking.
     *
    ...
    ulong seekCurrent(long offset)
    {
        // see if we are in read mode or not
        if(valid == valid.max)
        {
            // in write mode.  flush as long as offset is not 0.  If it's 0, do
            // a tell with the buffer offset.
            if(offset == 0)
                return _output.tell() + pos;

            // otherwise, flush, and seek the _output stream.
            _flush_noModeCheck();
            return _output.seekCurrent(offset);
        }
        else
        {
            // in read mode.  see if we can see within the buffer
            long target = pos + offset;
            if(target < 0 || target > decoded)
            {
                offset -= valid - pos;
                pos = valid = decoded = 0;
                return _input.seekCurrent(offset);
            }

            // can seek within our buffer
            pos = cast(size_t)target;

            if(_input.seekable())
                return _input.tell() - (valid - pos);
            return ulong.max;
        }
    }
    /// ditto
    ulong seekAbsolute(ulong offset)
    {
        if(valid == valid.max)
        {
            // write mode, make sure we flush
            _flush_noModeCheck();
            return _output.seekAbsolute(offset);
        }
        else
        {
            // reset the buffer and seek the underlying stream
            pos = valid = decoded = 0;
            return _input.seekEnd(offset);
        }
    }
    ...
    @property bool seekable()
    {
        ...
        {
            assert(_input)
            return _input.seekable();
        }
    }

    /**
     * Get the size of the valid data in the buffer.  For a read mode buffer,
     * this is the number of bytes seekable within the buffer.
     *
     * For a write mode buffer, this is the number of bytes ready to be written
     * to the stream.
     */
    @property size_t validData()
    {
        if(valid == valid.max)
            return decoded;
        else
    ...
    /**
     * Get the total buffer size, no matter what the mode is.
     */
     ...
     * buffer.
     *
     * For a stream in write mode, this always returns 0.
     */
    @property size_t readable()
    {
        return valid == valid.max ? 0 : decoded - pos;
    }

    /**
     * Is this stream reading or writing?
     */
    @property bool reading()
    {
        return valid != valid.max;
    }
    ...
    void close()
    {
        // this will handle resetting any positional data.
        flush();
        if(_input)
        {
            _input.close();
    ...
    /**
     * Get the underlying output stream for this buffered stream.
     */
...
BufferedStream buffered(string mode = "auto", T)
               (T obj,
                uint initialBufSize = PAGE * 10,
                size_t minReadSize = PAGE)

         if (mode == "auto" && (is(T : InputStream) || is(T : OutputStream)))
{
    static if(is(T : InputStream) && is(T : OutputStream))
        return new BufferedStream(obj, obj, initialBufSize, minReadSize);
    else static if(is(T : OutputStream))
        return new BufferedStream(null, obj, initialBufSize, minReadSize);
    else static if(is(T : InputStream))
        return new BufferedStream(obj, null, initialBufSize, minReadSize);
    else
        static assert(0, "Invalid stream object passed to buffered");
}

BufferedStream buffered(string mode, T)
               (T obj,
                initialBufSize = PAGE * 10,
                size_t minReadSize = PAGE)

         if (mode == "r" && is(T : InputStream))
{
    return new BufferedStream(obj, null, initialBufSize, minReadSize);
}
...
BufferedStream buffered(string mode, T)
         ...
         if (mode == "w" && is(T : OutputStream))
{
    return new BufferedStream(null, obj, initialBufSize, 0);
}
...
BufferedStream buffered(string mode, T)
         ...
         if (mode == "rw" && is(T : OutputStream) && is(T : InputStream))
{
    return new BufferedStream(obj, obj, initialBufSize, minReadSize);
}
...
BufferedStream buffered()
               (InputStream istream, OutputStream ostream,
                ...
                size_t minReadSize = PAGE)
{
    return new BufferedStream(istream, ostream, initialBufSize, minReadSize);
}
...
struct ByChunk
{
    ...
    {
        this(buffered(input), size);
    }

    this(BufferedStream dbi, size_t size)
    in
    {
