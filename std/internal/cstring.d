...
{
    static C[] asArray(C)(C* cstr) pure nothrow @nogc @trusted
        if (isSomeChar!C)
            in { assert(cstr); }
    do
    {
    ...
    assert(asArray("abc".tempCString()) == "abc");
    assert(asArray("abc"d.tempCString().ptr) == "abc");
    assert(asArray("abc".tempCString!wchar().buffPtr) == "abc"w);

    import std.utf : byChar, byWchar;
    char[300] abc = 'a';
    assert(asArray(tempCString(abc[].byChar).buffPtr) == abc);
    assert(asArray(tempCString(abc[].byWchar).buffPtr) == abc);
